<template>
  <div class="ofa-form">
    <el-card content-position="left" class="form-header" shadow="never">
      <el-page-header @back="goBack" content="系统通知 Notification" size="mini"></el-page-header>
      ...
      <el-card content-position="left" shadow="never">
        <el-form status-icon ref="form" :rules="validationRules" :model="entity" :disabled="disabled" class="form"
          ...
          <el-form-item label="类型" prop="Content">
            <el-radio-group v-model="entity.Type" @change="changeSelectType" size="mini">
            ...
            <div v-show="entity.Type == 2" class="account-tag-box">
              <el-tag closable v-for="item in accountSelectionList" :key="item.Id" @close="removeAccountSelection(item)"
                type="primary">
                {{item.UserName}}</el-tag>
            </div>
          </el-form-item>
          <el-form-item label="内容" prop="Content">
            <tinymce-editor v-if="!loading" :key="entity.Id" v-model="entity.Content" @uploadImage="uploadImage"
    ...
    <!-- 选择账户弹窗 -->
    <el-dialog append-to-body title="选择向指定账号发送通知" :visible.sync="accountDialogVisible" width="60%"
      :before-close="closeFixTeamDialog">
      <div class="fix-account-box">
        <search-input enterable v-model="accountSearchOption.key" @click="getAccounts" placeholder="输入用户名称或账号搜索"
          size="mini"></search-input>
        <div class="account-box">
          <el-table border :data="accountList" @selection-change="accountSelectionChange" ref="accountTable"
            row-key="Id" size="mini" height="300px">
            <el-table-column prop="UserName" label="账号列表" header-align="center">
              <template slot-scope="scope">
                {{scope.row.UserName}}&nbsp;
                <el-tag size="mini" type="primary">{{scope.row.TenantName}}</el-tag>
              </template>
            </el-table-column>
          ...
          <el-table border :data="accountSelectionList" row-key="Id" size="mini" height="300px">
