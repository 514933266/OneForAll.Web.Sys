<template>
  <div class="ofa-form">
    <el-card content-position="left" class="form-header" shadow="never">
      <el-page-header @back="goBack" :content="`${entity.Name}-管理 Manage`" size="mini"></el-page-header>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    get () {
      ...
      url = url.replace(/{id}/, this.entity.Id)
      this.axios.get(url).then(response => {
        this.permSelectionList = [...response]
        this.loading = false
      })
    },
    updateTenant () {
    ...
    bindPermission () {
      let url = this.$root.getApi(API.KEY, API.TENANT.PERMISSION)
      url = url.replace(/{id}/, this.entity.Id)
      this.axios.post(url, this.permSelectionList).then(response => {
        if (response.Status) {
          this.get()
        }
      })
