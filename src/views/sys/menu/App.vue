    ...
    <el-card shadow="never">
      <div class="ofa-work-box">
        <span>
          ...
          <el-button @click="showSortArrow = !showSortArrow" class="ofa-button ofa-button-success" size="mini">
            <font-awesome-icon fas icon="exchange-alt" rotation="90"></font-awesome-icon>&nbsp;{{showSortArrow ? '完成排序' : '调整排序'}}
          </el-button>
        </span>
        <span>
          <search-input enterable v-model="searchOption.key" placeholder="输入菜单名称搜索" size="mini"></search-input>
        </span>
      </div>
    </el-card>
    <el-card shadow="never" class="table-card">
      <el-table border v-loading="loading" :data="list" :tree-props="{children: 'Children'}" ref="table" row-key="Id"
        size="mini" class="menu-table" row-class-name="name-box">
        <el-table-column label="名称" prop="Name">
          <template slot-scope="scope">
            <label>{{scope.row.Name}}</label>
            <transition name="el-fade-in-linear">
              <div v-show="showSortArrow" class="name-arrow-box">
                <font-awesome-icon :key="scope.row.Id + 'arrow-up'" @click="sort(scope.row, true)" fas icon="arrow-up"
                  class="arrow-up">
                </font-awesome-icon>
                <font-awesome-icon :key="scope.row.Id + 'arrow-down'" @click="sort(scope.row, false)" fas
                  icon="arrow-down" class="arrow-down">
                </font-awesome-icon>
              </div>
            </transition>
          </template>
        ...
        <el-table-column label="类型" prop="Type" width="100" align="center" header-align="center">
          <template slot-scope="scope">
            <el-tag size="mini">{{types[scope.row.Type].label}}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="160" align="center" header-align="center">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="showCopyDialog(scope.row)" size="mini">克隆</el-button>
            <el-button type="text" class="ofa-text-danger" size="mini" @click="del(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 弹窗 -->
      <el-dialog :visible.sync="dialogVisible" :title="'克隆菜单到【' + copyMenu.Name + '】'" width="80%" size="mini">
