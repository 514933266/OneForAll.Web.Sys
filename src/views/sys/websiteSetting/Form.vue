<template>
  <div class="ofa-form">
    <el-card content-position="left" class="form-header" shadow="never">
      <el-page-header @back="goBack" content="网站设置 Website" size="mini"></el-page-header>
      ...
      <el-card content-position="left" shadow="never">
        ...
        <div class="row">
          <div class="left-column">
            <el-form status-icon ref="form" :disabled="disabled" :rules="validationRules" :model="entity"
              class="base-form" label-width="120px" size="mini">
              <div class="row">
                <el-form-item label="所属机构" prop="Name" class="row-item">
                  <el-select filterable remote :remote-method="getTenants" v-model.trim="entity.Name"
                    placeholder="请搜索后选择">
          ...
          <div class="right-column">
            ...
            <el-alert show-icon :closable="false" title="背景图仅能上传小于1MB的jpg、png格式图片" type="warning"
              style="margin-top:10px;"></el-alert>
          </div>
        </div>
      </el-card>
      <el-card v-show="!isAdd" content-position="left" shadow="never">
        <div slot="header">
          <span class="card-header-label">API域名</span>
        </div>
        <div style="margin-bottom:10px;">
          <el-button class="ofa-button" size="mini" @click="showApiDialog(true)">
            <font-awesome-icon fas icon="plus"></font-awesome-icon>&nbsp;新增
          </el-button>
          <el-button class="ofa-button" size="mini" @click="showApiDialog(false)">
            <font-awesome-icon fas icon="edit"></font-awesome-icon>&nbsp;修改
          </el-button>
          <el-button class="ofa-button ofa-button-danger" size="mini" @click="delApi">
            <font-awesome-icon fas icon="trash"></font-awesome-icon>&nbsp;删除
          </el-button>
        </div>
        <el-table border :data="apiList" @selection-change="apiSelectionChange" size="mini" class="api-table">
    ...
    <!-- 弹窗 -->
    <el-dialog :visible.sync="apiSettingDialogVisible" title="Api授权" size="mini">
      ...
      <el-form status-icon ref="apiForm" :rules="apiValidationRules" :model="apiEntity"
        label-width="120px" size="mini" style="margin-top:20px;">
        <div slot="header">
          <span class="card-header-label">基本信息</span>
        </div>
        <el-form-item label="名称" prop="Name" class="row-item">
          <el-select filterable remote :value="apiEntity.Name" :remote-method="getMenus" @change="selectMenu"
            size="mini" placeholder="请输入菜单名称后选择">
            <el-option v-for="item in menuList" :key="item.Id" :label="item.Name" :value="item.Id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="代码" prop="Code" class="row-item">
          <el-input v-model="apiEntity.Code" size="mini" placeholder="请输入菜单代码">
          </el-input>
        </el-form-item>
        <el-form-item label="域名" prop="Host" class="row-item">
          <el-input v-model="apiEntity.Host" size="mini" placeholder="请输入Api域名"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="apiSettingDialogVisible = false">取 消</el-button>
...
export default {
  ...
  methods: {
    ...
    goBack () {
      ...
      this.$root.navigate({ ...WEBSITE_SETTING, params: {} })
    }
  },
  created () {
    this.init()
